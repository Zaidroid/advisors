...
class AuthService {
  ...
  private async init() {
    try {
      // SSO: Check URL hash for tokens passed from Command Center
      if (window.location.hash.includes('access_token=')) {
        const hashParams = new URLSearchParams(window.location.hash.substring(1));
        const ssoToken = hashParams.get('access_token');
        if (ssoToken) {
          localStorage.setItem('google_access_token', ssoToken);
          if (hashParams.get('token_expiry')) localStorage.setItem('token_expiry', hashParams.get('token_expiry')!);
          if (hashParams.get('user_email')) localStorage.setItem('user_email', hashParams.get('user_email')!);
          // Clear hash
